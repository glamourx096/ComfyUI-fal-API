...
class GrokImagineImageEdit:
    ...
    def INPUT_TYPES(cls):
        return {
            ...
            "optional": {
                ...
                "sync_mode": ("BOOLEAN", {"default": False}),
                "quality_model": ("BOOLEAN", {"default": False}),
            },
        }
    ...
    def edit_image(
        ...
        try:
            if quality_model:
              result = ApiHandler.submit_and_get_result(
                "xai/grok-imagine-image/quality/edit", arguments
              )
            else:
              result = ApiHandler.submit_and_get_result(
                "xai/grok-imagine-image/edit", arguments
              )
...
class Wan27Edit:
    ...
    def INPUT_TYPES(cls):
        return {
            ...
            "optional": {
                ...
                "output_format": (["jpeg", "png", "webp"], {"default": "jpeg"}),
                "seed": ("INT", {"default": -1, "min": -1, "max": 2147483647}),
                "pro_model": ("BOOLEAN", {"default": False}),
            },
        }
    ...
    def edit_image(
        ...
        model_name = "Wan 2.7 Edit"

        # Upload all provided images (API requires 1-4 images)
        image_urls = []
        for i, img in enumerate([image_1, image_2, image_3, image_4], 1):
            if img is not None:
                url = ImageUtils.upload_image(img)
        ...
        if len(image_urls) == 0:
            print(f"Error: At least 1 image is required for {model_name}")
            return ResultProcessor.create_blank_image()
...
class LumaUni1Edit:
    ...
    def edit_image(
        ...
        model_name = "Luma Uni-1 Edit"

        # Upload the source image
        image_url = ImageUtils.upload_image(image)
        if not image_url:
            print(f"Error: Failed to upload source image for {model_name}")
            return ResultProcessor.create_blank_image()
        ...
        for i, ref_img in enumerate(
            ...
            if ref_img is not None:
                ref_url = ImageUtils.upload_image(ref_img)
                if ref_url:
                    reference_image_urls.append(ref_url)
                else:
                    print(
                        f"Error: Failed to upload reference image {i} for {model_name}"
                    )
                    return ResultProcessor.create_blank_image()
        ...
        try:
            result = ApiHandler.submit_and_get_result(
                "luma/agent/uni-1/v1/edit", arguments
            )
            return ResultProcessor.process_image_result(result)
        except Exception as e:
            return ApiHandler.handle_image_generation_error(model_name, e)
...
class JoyaiImageEdit:
    ...
    def edit_image(
        ...
        try:
            result = ApiHandler.submit_and_get_result(
                "fal-ai/joyai-image-edit", arguments
            )
            return ResultProcessor.process_image_result(result)
        except Exception as e:
            return ApiHandler.handle_image_generation_error(model_name, e)
...
class PhotaEdit:
    ...
    def INPUT_TYPES(cls):
        return {
            ...
            "optional": {
                ...
                "aspect_ratio": (
                    ["auto", "1:1", "16:9", "4:3", "3:4", "9:16"],
                    {"default": "auto"},
                ),
                "profile_ids": ("STRING", {"default": ""}),
            },
        }
    ...
    def edit_image(
        ...
        model_name = "Phota Edit"

        # Upload all provided images (API supports up to 10, extras are ignored)
        image_urls = []
        for i, img in enumerate(
            [
                image_1,
        ...
        # Parse comma-separated profile_ids string into a list
        if profile_ids:
            parsed_profiles = [
                pid.strip() for pid in profile_ids.split(",") if pid.strip()
            ]
            if parsed_profiles:
                arguments["profile_ids"] = parsed_profiles

        try:
            result = ApiHandler.submit_and_get_result(
                "fal-ai/phota/edit", arguments
            )
            return ResultProcessor.process_image_result(result)
        except Exception as e:
            return ApiHandler.handle_image_generation_error(model_name, e)
...
NODE_CLASS_MAPPINGS = {
    ...
    "GrokImagineImageEdit_fal": GrokImagineImageEdit,
    "Wan27Edit_fal": Wan27Edit,
    "LumaUni1Edit_fal": LumaUni1Edit,
    "JoyaiImageEdit_fal": JoyaiImageEdit,
    "PhotaEdit_fal": PhotaEdit,
}
...
NODE_DISPLAY_NAME_MAPPINGS = {
    ...
    "GrokImagineImageEdit_fal": "Grok Imagine Image Edit (fal)",
    "Wan27Edit_fal": "Wan 2.7 Edit (fal)",
    "LumaUni1Edit_fal": "Luma Uni-1 Edit (fal)",
    "JoyaiImageEdit_fal": "Joyai Image Edit (fal)",
    "PhotaEdit_fal": "Phota Edit (fal)",
}
