...
def _has_audio_stream(file_path):
    """Check whether the given media file contains an audio stream using ffprobe."""
    try:
...
def _extract_audio(file_path):
    """Extract audio from a media file as a VHS-compatible AUDIO dict.

    Returns ``{"waveform": tensor, "sample_rate": int}`` or ``None`` if extraction fails.
    The waveform tensor has shape (1, channels, samples) and dtype float32, matching
    what ``VHS_VideoCombine`` expects on its ``audio`` input.
    """
    try:
        # Dump raw 32-bit float little-endian PCM to stdout
        proc = subprocess.run(
    ...
    if match:
        ...
        if channel_label in ("mono", "stereo"):
            channels = 1 if channel_label == "mono" else 2
        else:
            # VHS only supports mono/stereo; fall back to a sensible default
            channels = 2
    else:
        sample_rate = 44100
        channels = 2
    ...
    try:
        waveform = torch.frombuffer(bytearray(proc.stdout), dtype=torch.float32)
...
class LoadVideoURL:
    ...
    RETURN_TYPES = ("IMAGE", "INT", "VHS_VIDEOINFO", "AUDIO")
    ...
    def load_video_from_url(
        ...
        # Extract audio (compatible with VHS_VideoCombine) before deleting the temp file
        audio = None
        if _has_audio_stream(temp_file_path):
            audio = _extract_audio(temp_file_path)
...
class Cosmos3SuperImageToVideoNode:
    @classmethod
    def INPUT_TYPES(cls):
        return {
            "required": {
                "prompt": ("STRING", {"default": "", "multiline": True}),
                "image": ("IMAGE",),
                "width": ("INT", {"default": 832, "min": 256, "max": 1920, "step": 8}),
            ...
            "optional": {
                ...
                "agentic_max_iterations": ("INT", {"default": 2, "min": 1, "max": 3}),
                "agentic_samples_per_iteration": ("INT", {"default": 2, "min": 1, "max": 3}),
                "agentic_early_stop": ("BOOLEAN", {"default": True}),
                "num_frames": ("INT", {"default": 189, "min": 5, "max": 189}),
                "frames_per_second": ("INT", {"default": 24, "min": 4, "max": 60}),
                "num_inference_steps": ("INT", {"default": 28, "min": 1, "max": 50}),
                "guidance_scale": ("FLOAT", {"default": 6.0, "min": 0.0, "max": 20.0, "step": 0.1}),
                "seed": ("INT", {"default": -1, "min": -1, "max": 2147483647}),
                "enable_safety_checker": ("BOOLEAN", {"default": True}),
    ...
    def generate_video(
        ...
        try:
            ...
            if not image_url:
                return ApiHandler.handle_video_generation_error(
                    "nvidia/cosmos-3-super/image-to-video", "Failed to upload image"
                )
            ...
            arguments = {
                ...
                "agentic_samples_per_iteration": agentic_samples_per_iteration,
                "agentic_early_stop": agentic_early_stop,
                "num_frames": num_frames,
                "frames_per_second": frames_per_second,
                "num_inference_steps": num_inference_steps,
                "guidance_scale": guidance_scale,
            ...
            result = ApiHandler.submit_and_get_result(
                "nvidia/cosmos-3-super/image-to-video", arguments
            )
            video_url = result["video"]["url"]
            return (video_url,)
        except Exception as e:
            return ApiHandler.handle_video_generation_error(
                "nvidia/cosmos-3-super/image-to-video", str(e)
            )


class GrokImagineVideo15Node:
    @classmethod
    def INPUT_TYPES(cls):
        return {
            "required": {
    ...
    def generate_video(self, prompt, image, duration, resolution, seed=-1):
        try:
            image_url = ImageUtils.upload_image(image)
            if not image_url:
                return ApiHandler.handle_video_generation_error(
            ...
            result = ApiHandler.submit_and_get_result(
                "xai/grok-imagine-video/v1.5/image-to-video", arguments
            )
            video_url = result["video"]["url"]
            return (video_url,)
        except Exception as e:
            return ApiHandler.handle_video_generation_error(
                "xai/grok-imagine-video/v1.5/image-to-video", str(e)
            )
...
class Seedance2ReferenceToVideoNode:
    ...
    @staticmethod
    def _save_audio_to_wav(audio, file_path):
        """Save ComfyUI AUDIO dict (waveform, sample_rate) to a WAV file."""
        try:
            ...
            # scipy.io.wavfile expects (samples,) for mono or (samples, channels)
            if waveform.ndim == 2:
                waveform = waveform.T
            ...
            print(f"Error saving audio to WAV: {str(e)}")
            return None
                ...
                for a in audio_urls:
                    ...
                    try:
                        with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as tf:
                            tmp_path = tf.name
                        saved = self._save_audio_to_wav(a, tmp_path)
                        if saved:
                            aud_url = ImageUtils.upload_file(saved)
                            if aud_url:
                                aud_urls.append(aud_url)
                    finally:
                        if tmp_path and os.path.exists(tmp_path):
                            os.unlink(tmp_path)
                if aud_urls:
                    arguments["audio_urls"] = aud_urls
...
NODE_CLASS_MAPPINGS = {
    ...
    "Cosmos3SuperImageToVideo_fal": Cosmos3SuperImageToVideoNode,
...
NODE_DISPLAY_NAME_MAPPINGS = {
    ...
    "Cosmos3SuperImageToVideo_fal": "Cosmos 3 Super Image-to-Video (fal)",
